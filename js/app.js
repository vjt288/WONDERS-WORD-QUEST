...
function totalStars(){return Object.values(state.mastery||{}).reduce((sum,m)=>sum+(m.star||0),0)}
function screen(html){app.innerHTML=`<div class="game"><div class="top"><div class="logo">⚔ WONDERS WORD QUEST</div><div class="stats"><span class="pill">🎯 ${state.score}</span><span class="pill">⭐ ${totalStars()}</span><span class="pill">🔥 ${state.combo}</span></div></div><main class="main">${html}</main></div>`;}
function menu(){
...
function nextQuestion(){
 ...
 <div class="timerWrap"><div class="timer"><div id="bar"></div></div><span class="timerText" id="timerText">30s</span></div><div class="question">${q.prompt}</div>${body}<div id="feedback" class="feedback"></div>
...
function startTimer(){
 ...
 state.timer=setInterval(()=>{
   const left=Math.max(0,state.deadline-Date.now()),pct=left/30000*100,seconds=Math.ceil(left/1000);
   const bar=$("#bar");if(bar)bar.style.width=pct+"%";
   const timerText=$("#timerText");if(timerText)timerText.textContent=seconds+"s";
