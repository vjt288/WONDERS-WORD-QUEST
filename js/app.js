...
const state={
  data:[], player:"YOU", character:null, chapter:0, mode:null, q:0, total:10,
...
function menu(){
 ...
 <div class="buttons">
 <button class="btn" id="continue">▶ CONTINUE ADVENTURE</button>
 <button class="btn secondary" id="map">🗺️ WORLD MAP</button>
 <button class="btn secondary" id="review">🔄 REVIEW QUEST</button>
 <button class="btn secondary" id="book">📖 MY WORD BOOK</button>
 <button class="btn secondary" id="ach">🏆 ACHIEVEMENTS</button>
 </div>
...
function questionFor(w){
 if(state.mode===0)return {title:"SPELL IT",prompt:`Spell this word: <strong>${esc(w.meaning)}</strong>`,kind:"input"};
 if(state.mode===1)return {title:"PICTURE MATCH",prompt:`Which word matches the picture?`,kind:"pic"};
 if(state.mode===2)return {title:"WORD → MEANING",prompt:`What does <strong>${esc(w.word)}</strong> mean?`,kind:"meaning"};
 if(state.mode===3)return {title:"SCRAMBLE",prompt:"Put the letters in the correct order.",kind:"scramble"};
 if(state.mode===4)return {title:"SENTENCE QUEST",prompt:`Choose the best word for this context: <br><strong>“The children saw a ${esc(w.word)} and learned about it.”</strong>`,kind:"choice"};
 return {title:"WORD ATTACK",prompt:`Quick! What is the meaning of <strong>${esc(w.word)}</strong>?`,kind:"meaning"};
}
function nextQuestion(){
 ...
 if(q.kind==="meaning"||q.kind==="choice"||q.kind==="pic"){
   const opts=q.kind==="pic"?shuffle([w.word,...distractors(w.word)]):shuffle([w.meaning,...distractors(w.word).map(x=>state.data.flatMap(c=>c.words).find(z=>z.word===x)?.meaning||x)]);
...
function submit(raw,timeout=false){
 ...
 const correct=norm(raw)===norm(w.word);
 const fb=$("#feedback");
